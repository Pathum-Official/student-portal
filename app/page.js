...
export default function Login() {
  ...
  const handleLogin = async (e) => {
    ...
    if (data && data.password === password) {
      ...
      alert("වැරදි ශිෂ්‍ය අංකයක් හෝ මුරපදයක්!");
    }
    setLoading(false);
  };

  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gradient-to-br from-slate-900 via-blue-900 to-slate-900 font-sans">
      ...
      <div className="z-10 w-full max-w-md p-10 bg-white/10 backdrop-blur-xl border border-white/20 shadow-2xl rounded-[2.5rem]">
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-3xl font-black text-white tracking-tight">ශිෂ්‍ය පිවිසුම</h1>
          <p className="text-blue-200 mt-2 text-sm font-medium">ඔබගේ ගිණුමට ඇතුළත් වන්න</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          <div className="relative">
            ...
            <input 
              required
              type="text" placeholder="ශිෂ්‍ය අංකය (ST001)" 
          ...
          <div className="relative">
            ...
            <input 
              required
              type="password" placeholder="මුරපදය" 
              className="w-full pl-12 pr-4 py-4 bg-black/20 border border-white/10 rounded-2xl text-white placeholder-blue-200/50 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all font-medium"
          ...
          <button disabled={loading} className="w-full py-4 mt-4 text-white bg-blue-600 hover:bg-blue-500 rounded-2xl font-black text-lg tracking-wide shadow-lg shadow-blue-600/30 transition-all transform hover:scale-[1.02] active:scale-95 disabled:opacity-50">
            {loading ? "ඇතුළත් වෙමින් පවතී..." : "පිවිසෙන්න"}
          </button>
        </form>
      </div>
    </div>
